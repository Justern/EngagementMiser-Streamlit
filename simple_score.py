...
try:
    ...
    print("PyTorch not available, using keyword-based scoring", file=sys.stderr)
...
def calculate_authority_signal_manipulation_score(text: str, model, tokenizer, device) -> float:
    ...
    if PYTORCH_AVAILABLE and model is not None:
        try:
            ...
            print(f"RoBERTa scoring failed, using fallback: {e}", file=sys.stderr)
...
def main():
    ...
    try:
        ...
        # Load the Hugging Face model (if available)
